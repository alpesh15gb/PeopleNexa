...
Upload the CSV, select the mapped leave type and matching cutoff month, run the mandatory dry run, resolve every reported error, then confirm. Confirmation stores immutable snapshots only: it never creates leave requests or changes policy allocations. The server hashes the original bytes with SHA-256 and treats an already-imported `(tenant, source hash, leave type, cutoff)` as idempotent. It also blocks unmatched employees, policy allocations, conflicting snapshots, malformed rows, and reconciliation failures.
