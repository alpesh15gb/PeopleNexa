...
export async function GET(request: NextRequest) {
  ...
  const employee = await prisma.employee.findFirst({ where: { tenantId: session.tenantId, deviceCode }, select: { employeeNumber: true, deviceCode: true, firstName: true, lastName: true, position: true, joiningDate: true, phone: true, profilePicture: true, profile: { select: { bloodGroup: true } } } });
