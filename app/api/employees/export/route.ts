...
const HEADERS = [
  ...
  "branch", "department", "shiftId", "payMode", "workBasisRate", "managerId", "status", "bankName", "accountNumber", "ifscCode", "pan", "uan", "aadhaarNumber", "drivingLicenseNumber",
] as const;
...
export async function GET() {
  ...
  const employees = await prisma.employee.findMany({
    ...
    select: {
      ...
      joiningDate: true, payMode: true, workBasisRate: true, managerId: true, status: true, bankName: true, accountNumber: true, ifscCode: true, pan: true, uan: true, aadhaarNumber: true, drivingLicenseNumber: true,
  ...
  const rows = employees.map((employee) => [
    ...
    employee.payMode, employee.workBasisRate, employee.managerId, employee.status, employee.bankName, employee.accountNumber, employee.ifscCode, employee.pan, employee.uan, employee.aadhaarNumber, employee.drivingLicenseNumber,
  ].map(cell).join(","));
