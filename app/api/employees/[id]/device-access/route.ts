...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const employee = await prisma.employee.findFirst({ where: { id, tenantId: session.tenantId }, select: { id: true, deviceCode: true, deviceAccessEnabled: true } });
  ...
  return NextResponse.json({
    ...
    deviceIds: access.filter((row) => row.allowed).map((row) => row.deviceId),
  });
}
...
export async function PUT(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const devices = await prisma.device.findMany({ where: { tenantId: session.tenantId, config: { path: ["ebioserver"], equals: true } }, select: { id: true, serialNumber: true, name: true } });
  ...
  await prisma.$transaction([
    ...
    prisma.employeeDeviceAccess.deleteMany({ where: { employeeId: id, tenantId: session.tenantId } }),
