...
export function IdCardPreviewSide({ title, employee, branding, template, front = false, photoPosition = { x: 50, y: 50 }, validTill }: { title: string; employee: Employee; branding: CompanyBranding | null; template: IdCardTemplate | null; front?: boolean; photoPosition?: { x: number; y: number }; validTill?: string | null }) {
  ...
  return <section><h2 className="mb-3 text-sm font-semibold">{title}</h2><div className="relative mx-auto w-full max-w-[306px] overflow-hidden bg-white shadow-lg" data-testid="id-card-preview-canvas" style={{ aspectRatio: ID_CARD_ARTBOARD.width / ID_CARD_ARTBOARD.height, overflow: "hidden", containerType: "inline-size" }}>
    <img src={background} alt="" aria-hidden="true" className="absolute inset-0 h-full w-full" data-testid="id-card-artwork" style={{ objectFit: ID_CARD_LAYOUT.background.fit }} />
    {front && <>
